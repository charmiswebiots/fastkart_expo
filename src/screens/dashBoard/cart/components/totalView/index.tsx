...
export const TotalView: FC<TotalViewProps> = ({ onPress }) => {
  const { colors } = useTheme();
  const { t, isDark } = useValues();
  
  return (
    <View style={styles.total as ViewStyle}>
      <Total title={t('cartPage.orderDetails')} style={{ color: isDark ? appColors.white : colors.text }} />
      <Button text={t('commonTotal.proceedtoCheckout')} style={styles.btn} color={appColors.white} onPress={onPress} />
    </View>
  );
}