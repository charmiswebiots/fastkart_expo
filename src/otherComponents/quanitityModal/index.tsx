...
// export function QuantityModal ({onPress}:QuantityModalProps)  {
//     const { colors } = useTheme();
//     const { isDark, viewRtlStyle, textRtlStyle, t } = useValues();
//     const [selectQty, setSelectQty] = useState<number>(0);

//     const selectQuantity = (key: number): void => {
//         setSelectQty(key);
//     };
        ...
        <View style={[GlobalStyle.modal, { backgroundColor: colors.background }]}>
            <FlatList
                ...
                renderItem={({ item, index }) => (
                    <TouchableOpacity
                        activeOpacity={0.8}
                        onPress={() => selectQuantity(index)}
                        style={[
                            {
                                backgroundColor: isDark ? colors.primary : appColors.gray,
                                borderColor: selectQty === index ? appColors.primary : appColors.gray,
                            },
                            styles.view,
                        ]}
                    >
                        {selectQty === index && <Selected />}
