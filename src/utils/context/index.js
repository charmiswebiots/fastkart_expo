...
//           setRtl(savedLanguage === 'ar' ? true : false); // Set RTL for Arabic
//         }
//       } catch (error) {
...
export const AppContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const savedSymbol = await AsyncStorage.getItem('currSymbol');
        if (savedSymbol) setCurrSymbolState(savedSymbol);

        const savedValue = await AsyncStorage.getItem('currValue');
        if (savedValue) setCurrValueState(Number(savedValue));
      } catch (error) {
        console.error('Error retrieving settings:', error);
      }
    };
  ...
  const setCurrValue = async (value) => {
    try {
      ...
      console.error('Error setting currency value:', error);
    }
  };
